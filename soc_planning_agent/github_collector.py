...
def analyze_trend_article(client: anthropic.Anthropic, article: dict) -> dict:
    ...
    prompt = (
        ...
        f"Use exactly these four sections with the specified focus:\n\n"
        f"## 目標對象\n"
        f"列出直接客戶與終端用戶，區分主要/次要目標，說明他們的角色與決策權。\n\n"
        f"## 創造的價值 × 解決的痛點\n"
        f"以每個目標對象為主角，分析：\n"
        f"  - 他們目前面對什麼具體痛點？現有方案有何不足？\n"
        f"  - 這個新產品/技術如何解決？帶來什麼量化價值（速度/成本/效率改善幅度）？\n"
        f"用表格或條列式，確保每個目標對象都有對應的痛點與解決方案。\n\n"
        f"## 商業模式\n"
        f"從目標對象出發，分析：\n"
        f"  - 為解決這個痛點，目標對象願意支付多少？付費方式（一次性/訂閱/授權費/服務費）？\n"
        f"  - 這個新技術/產品帶動哪些新的商業行為或採購決策？\n"
        f"  - 市場規模估計（TAM/SAM）與定價模型。\n\n"
        f"## 產業鏈誘因\n"
        f"從目標對象往上游及相關 ecosystem 逐層分析：\n"
        f"  - 各層（IP廠商→晶片設計→Foundry→OEM→電信業者→消費者）是否有利可圖？\n"
        f"  - 哪些廠商會積極支持？為什麼？\n"
        f"  - 是否存在利益衝突？哪些現有廠商會受損或抵制？\n"
        f"  - 這個技術/產品是否破壞現有產業鏈結構？\n"
        f"  - 既有廠商（incumbents）的態度：抵抗/適應/主導？\n"
        f"  - 新進入者的能力與機會窗口。\n"
        f"用清楚的條列或表格，避免模糊的描述。"
    )
