...
找出 2-3 個最重要的結構性機會。每個機會**必須按順序**回答以下四個因果問題，每一層的第一行就是該層的因果問題，必須直接回答，不能跳過：

**🔵 啟動層**
> ❓ 因果問題（必答）：為何這個新趨勢造成了這個新機會？趨勢改變了什麼條件（技術可行性／成本曲線／法規／供需），使得原本不存在的機會現在出現？
- 趨勢描述：（引用具體文章 [來源 日期]）
- 因果回答：（說明機制；說不清楚機制則標 [未驗證] 並說明缺什麼資訊）

**🟡 鎖定層**
> ❓ 因果問題一（必答）：這個新機會為何讓某個客群現在可以完成過去做不到的 task？機會提供了什麼新條件（新工具／新成本結構／新生態），讓這個 task 首次變得可行？
> ❓ 因果問題二（必答）：為何這個目標客群在完成這個 task 時，會遭遇過去一直無法解決的痛點？現有方案的哪個具體缺陷（速度／精度／成本／整合度）阻止了他們？
- 因果回答一：（說明新機會如何解鎖 task）
- 因果回答二：（說明痛點的具體機制，不能只說「需求大」）
...
**🔴 收成層**
> ❓ 因果問題（必答）：為何這個商業模式可以將上層創造的客戶價值轉化為可持續的商業利益？定價權從哪裡來？客戶為何不轉向競品？護城河是什麼，會隨時間加深還是消失？
- 因果回答：（說明價值捕獲機制）
