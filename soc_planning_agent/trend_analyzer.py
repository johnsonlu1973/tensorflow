...
- 導出說明：（從上述觀察，說明為何可以歸納出一個宏觀結構性轉變；需逐步推論，不能跳躍。格式：觀察A＋觀察B → 共同指向 XXX 方向 → 這代表 XXX 遊戲規則正在改變，因為…）
- 產業趨勢：（根據上方導出說明，陳述趨勢；再確認三個條件：① 宏觀 — 影響整個產業而非單一公司；② 長期 — 5年以上驅動力；③ 不可逆 — 即使短期受阻仍會繼續；任一條件無法確認，標 [未驗證]）
